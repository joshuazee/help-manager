...
namespace CMHL.Lawer
{
    ...
    public class MultipartParser
    {
        ...
        public MultipartParser(Stream stream, string directory, string type)
        {
            switch(type)
            {
                case "image":
                    break;
                default:
                    throw new Exception("未支持的文件上传格式");
            }
        }
        ...
        public MultipartParser(string ImageContent)
        {
            string[] contents = Regex.Split(ImageContent, "###");
            List<string> list = new List<string>();
            for (int i = 0; i < contents.Length; i++)
            {
                string content = contents[i];
                string filename = "NewsImage\\" + Guid.NewGuid().ToString();
                int index = content.IndexOf("image/") + 6;
                string imgType = content.Substring(index, content.IndexOf(";", index) - index);
                filename += ImageTypeToExt(imgType);
                byte[] bytes = Convert.FromBase64String(content.Split(',')[1].Replace(" ", "+"));
                using (MemoryStream stream = new MemoryStream(bytes))
                {
                    Bitmap bmp = new Bitmap(stream);
                    bmp.Save(AppHome.Upload + filename);
                    stream.Close();
                }
                list.Add(filename.Replace("\\", "/"));
            }

            imagePaths = list.ToArray();
        }
        private string ImageTypeToExt(string type)
        {
            string ext = "";
            switch (type)
            {
        ...
        private void Parse(Stream stream, Encoding encoding)
        {
            ...
            if (delimiterEndIndex > -1)
            {
                ...
                if (contentTypeMatch.Success && filenameMatch.Success)
                {
                    ...
                    int mystartIndex = IndexOf(data, encoding.GetBytes(contentTypeMatch.ToString() + "\r\n\r\n"), contentTypeMatch.Index);
                    mystartIndex += contentTypeMatch.Length + "\r\n\r\n".Length;
                    // Get the start & end indexes of the file contents
                    int startIndex = contentTypeMatch.Index + contentTypeMatch.Length + "\r\n\r\n".Length;

                    byte[] delimiterBytes = encoding.GetBytes("\r\n" + delimiter);
                    int endIndex = IndexOf(data, delimiterBytes, mystartIndex);

                    int contentLength = endIndex - mystartIndex;

                    // Extract the file contents from the byte array
                    byte[] fileData = new byte[contentLength];

                    Buffer.BlockCopy(data, mystartIndex, fileData, 0, contentLength);
                    this.con = encoding.GetString(fileData);
